...
def setup():
  ...
  M5.begin()
  title0 = Widgets.Title("Digital Level", 3, 0x000000, 0xffffff, Widgets.FONTS.DejaVu18)
...
# function to change hue, saturation, brightness into r, g, b:
...
def loop():
  global title0, label0, label1, label2
  global imu_val, imu_x_val, imu_x_last
  global rgb, adc1
  global update_timer, motion_timer, program_state
  global rgb_color, r, g, b, r_final, g_final, b_final
  global rgb_step  # how fast to fade color
  ...
  imu_x_last = imu_x_val  # update last IMU X value
  imu_x_val = imu_val[0]  # update IMU X value
  imu_x_diff = imu_x_val - imu_x_last  # difference between new and last IMU X value
  ###
  imu_deg = map_value(imu_x_val, -1.0, 1.0, -90, 90)
  # show X-axis value:
  imu_str = 'Degree: {:0.2f}'.format(imu_deg)
  label0.setText(str(imu_deg))
  
  led_x = map_value(imu_x_val, -1.0, 1.0, 0, 13)
  #label1.setText(str(led_x))
  
  label1.setText(program_state)
  
  rgb.fill_color(0)  # turn off all pixels
  rgb.set_color(6, rgb_color)  # center (6th) pixel always on
  
  if led_x < 6:
    for i in range(5, led_x-1, -1):  # (start, stop, step)
      rgb.set_color(i, rgb_color)
    #time.sleep_ms(200)
  ...
  if program_state == 'IDLE':
    ...
    #rgb_color = hsb_to_color(hue_val, 255, 255)
    rgb_final = hsb_to_rgb(hue_val, 255, 255)
    r_final = rgb_final[0]
    g_final = rgb_final[1]
    b_final = rgb_final[2]
    rgb_step = 1  # smaller fade step 
    
    # change from IDLE to MOTION state when motion is detected:
    #if imu_x_diff > 0.25 or imu_x_diff < -0.25:
    if imu_x_diff > 0.1 or imu_x_diff < -0.1:
    ...
    # set color to be red:
    #rgb_color = get_color(255, 0, 0)
    #rgb_final = get_color(255, 0, 0)
    r_final = 20
    g_final = 250
    b_final = 0
    rgb_step = 5  # bigger fade step
...
#   rgb.set_color(led_x, get_color(r, g, b))
  ...
  # make sure r, g, b stay in 0 - 255 range:
  if r > 255:
    r = 255
  elif r < 0:
    r = 0
